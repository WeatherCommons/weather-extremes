# All-Time Records — ${observable.params.country}

Records are reported in **°C** for temperature and **mm** for rainfall.

```js
const records = await FileAttachment(`/data/atr/${observable.params.country}.csv`).csv({typed: true});
```
...
display(recordsTable(filtered, ["Month", "City", "Country", "Metric", "Type", "Value", "Date", "Scope", "Since"]));
```
